...
class MakeCurrencies extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'make:currencies {alias} {currencies_list} {--em=} {--pm=}';
     ...
     * @var string
     */
    protected $description = 'Make currencies list for foundation by alias and currencies codes example:<USD,EUR,RUB>';
    ...
    {
        $command_title = 'Make Currencies';
        // Chacking Foundation And validation Of Parametrs
        try  {

            $alias = trim(strval($this->argument('alias')));
            $currencies_list = explode(",", strval($this->argument('currencies_list')));
            ...
            $wrongParams = Helpers::validationCommandParametrs(
                ['alias'=>$alias, 'currencies'=>$currencies_list, 'email'=>$moderator_email, 'passwd'=>$moderator_passwd]
            );
        ...
        try {
            $this->line('--- Attache currencies for foundation ('. $alias.')');
            
            $bar = $this->output->createProgressBar(count($currencies_list));
            $bar->start();

            $srcCurrencies = Currency::all();
            $fnd->currencies()->detach();

            foreach ($currencies_list as $currency) {
                
                $selected_currency = $srcCurrencies->firstWhere('code', $currency);

                $fnd->currencies()->attach($selected_currency, [
                    'added_at'      => now(),
