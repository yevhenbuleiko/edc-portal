...
use App\Models\Foundation\Decor; 
use App\Models\User;

use Config;
use Helpers;

class MakeDecors extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'make:decors {alias} {--em=} {--pm=}';
     ...
     * @var string
     */
    protected $description = 'Make decor for foundation by alias';
    ...
    {
        $command_title = 'Make Decors For Foundation';
        ...
        try {
            $this->line('--- Create decors for foundation ('. $alias.')');

            $decors = Config::get('seedsdata.decors');

            $bar = $this->output->createProgressBar(count($decors));
            $bar->start();

            foreach ($fnd->decors as $dcv) {
                $dcv->delete();
            }

            foreach ($decors as $dcr) {
                $fnd->decors()->create([
                    'title'      => $dcr[0],
                    'by'         => $dcr[2],
                    'value'      => $dcr[1],
